...
local function MakeRow(rownames, idx)
	...
	return Def.ActorFrame{
		...
		LoadFont("_avenirnext lt pro bold Bold 20px")..{
			InitCommand=cmd(x,64;uppercase,true;zoom,0.9);
		...
		LoadActor("cursor")..{
			InitCommand=cmd(zoom,1.5;x,-20;diffusealpha,1;bounce;effectmagnitude,3,0,0;effectperiod,1);
			GainFocusCommand=cmd(visible,true);
			LoseFocusCommand=cmd(visible,false);
		};
		LoadActor("cursor")..{
			InitCommand=cmd(zoom,1.5;x,146;diffusealpha,1;zoomx,-1.5;bounce;effectmagnitude,-3,0,0;effectperiod,1);
			GainFocusCommand=cmd(visible,true);
			LoseFocusCommand=cmd(visible,false);
		};
	};
	...
	Def.ActorScroller{
        ...
        ChangeRowMessageCommand=function(s,param)
            ...
            if param.PlayerNumber == pn then
                s:SetCurrentAndDestinationItem(screen:GetCurrentRowIndex(param.PlayerNumber))
            end
		end
	};
	Def.ActorFrame{
